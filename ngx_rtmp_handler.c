...
void ngx_rtmp_reset_ping(ngx_rtmp_session_t *s) {
  ...
  ngx_log_error(NGX_LOG_INFO, s->connection->log, 0, "ping: wait %Mms",
                cscf->ping);
}
...
static void ngx_rtmp_ping(ngx_event_t *pev) {
  ...
  if (s->ping_reset) {
    ngx_log_error(NGX_LOG_INFO, c->log, 0, "ping: i/o reset timer");
    ngx_rtmp_reset_ping(s);
    return;
  }
  ...
  if (ngx_rtmp_send_ping_request(s, (uint32_t)ngx_current_msec) != NGX_OK) {
    ngx_log_error(NGX_LOG_INFO, c->log, 0, "ping: failed sent ping");
    ngx_rtmp_finalize_session(s);
    return;
  } else {
    ngx_log_error(NGX_LOG_INFO, c->log, 0, "ping: sent ping");
  }
  ...
  for (;;) {
    ...
    if (old_size) {
      ...
      if (s->ack_size && s->in_bytes - s->in_last_ack >= s->ack_size) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_RTMP, c->log, 0, "sending RTMP ACK(%uD)",
                       s->in_bytes);
    ...
    if (b->pos == b->start) {
      ...
      if (csid == 0) {
        if (b->last - p < 1) continue;
        csid = 64;
        csid += *(uint8_t *)p++;

      } else if (csid == 1) {
        if (b->last - p < 2) continue;
        csid = 64;
        csid += *(uint8_t *)p++;
    ...
    if (size < ngx_min(fsize, s->in_chunk_size)) continue;
...
ngx_int_t ngx_rtmp_send_message(ngx_rtmp_session_t *s, ngx_chain_t *out,
  ...
  if (nmsg + priority * s->out_queue / 4 >= s->out_queue) {
    ngx_log_debug2(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                   "RTMP drop message bufs=%ui, priority=%ui", nmsg, priority);
    return NGX_AGAIN;
  }
    ...
    for (n = 1; n < cscf->max_streams; ++n) {
      ...
      for (;;) {
        ...
        if (bo->end - bo->last >= bi->last - bi->pos) {
          bo->last = ngx_cpymem(bo->last, bi->pos, bi->last - bi->pos);
          li = li->next;
          if (li == fli) {
            lo->next = flo;
        ...
        bi->pos +=
            (ngx_cpymem(bo->last, bi->pos, bo->end - bo->last) - bo->last);
